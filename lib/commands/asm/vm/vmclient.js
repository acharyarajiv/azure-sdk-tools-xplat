...
_.extend(VMClient.prototype, {
      ...
      if (error) {
        ...
        if (found) {
          ...
          if (options.quiet)
            ...
            self.cli.interaction.confirm(util.format($('Delete the VM %s ? [y/n] '), vmName), function (dummy, shouldDelete) {
              if (shouldDelete) {
                deleteVMInternal();
              } else {
                return callback();
      ...
      if (error) {
        ...
        if (found) {
          ...
          if (options.attach) {
            ...
            diskInfo.hostCaching = 'ReadWrite';
  ...
  createDockerVM : function (dnsName, imageName, userName, password, options, callback, logger) {
    ...
    return createDockerVM(dnsName, options, logger, callback);
  }
});
  ...
  if (options.role) {
    ...
    if (options.sshCert) {
      ...
      loadSshCert(options, logger, function (loadSshErr, newPemSshCert, newSshFingerprint) {
        ...
        createHostedService(dnsPrefix, options, logger, function (hostedServiceError, alreadyExists) {
    ...
    if (options.community) {
      ...
      communityUtil.resolveUid(options.imageName, function (error, response) {
        ...
        if (!error) {
          ...
          verifyUserNameAndPwd('linux', options, logger, function (error) {
            if (error) {
              return callback(error);
            }
      ...
      lookupImage(options.computeManagementClient, options.imageName, logger, function (imgErr, foundImage) {
        ...
        image = foundImage;
        verifyUserNameAndPwd(image.operatingSystemType, options, logger, function (error) {
          if (error) {
            return callback(error);
          }
          ...
          if (error) {
            if (error.statusCode === 404) {
              ...
              copyAndRegCommunityImgIfRequired(function () {
                if (!role) {
                  createRole(null, dnsPrefix, image, options, logger, function (createRoleError, newRole) {
                    if (createRoleError) {
            ...
            copyAndRegCommunityImgIfRequired(function () {
              if (!hookEx) {
                if (!role) {
                  ...
                  vmName += tag;
                  createRole(vmName, dnsPrefix, image, options, logger, function (createRoleError, newRole) {
                    if (createRoleError) {
                      callback(new Error(createRoleError));
                    }
...
function createDockerVM(dnsName, options, logger, callback) {
  ...
  checkAndGenerateCertificatesIfNeeded(function (certificateError) {
    ...
    lookupImage(options.computeManagementClient, options.imageName, logger, function (imgErr, image) {
      if (imgErr) {
        return callback(imgErr);
      }
      ...
      verifyUserNameAndPwd(image.operatingSystemType, options, logger, function (error) {
        ...
        verifyCertFingerPrint(image.operatingSystemType, options, logger, function (certErr, pemSshCert, sshFingerprint) {
          ...
          createHostedService(options.dnsPrefix, options, logger, function (hostedServiceError) {
            if (hostedServiceError) {
              return callback(hostedServiceError);
            }

            createRole(dnsName, options.dnsPrefix, image, options, logger, function (createRoleError, role) {
              if (createRoleError) {
                callback(new Error(createRoleError));
              }
    ...
    utils.fileExists(options.dockerCertDir, function (certDirError, exists) {
      ...
      if (!exists) {
        ...
        fs.mkdir(options.dockerCertDir, function (mkdirErr) {
          ...
          var progress = self.cli.interaction.progress($('Generating docker certificates.'));
...
function createRole(name, dnsPrefix, image, options, logger, callback) {
  var inputEndPoints = [];
  ...
  if (options.imageTarget || image.category !== 'User') {
    blobUtils.getBlobName(self.cli, options.storageClient, options.location, options.affinityGroup, dnsPrefix, options.imageTarget,
...
function verifyUserNameAndPwd(osName, options, logger, callback) {
